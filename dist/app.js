...
function runAction(kind, label, mutator) {
...
function undoLast() {
    for (let i = journal.length - 1; i >= 0; i--) {
        ...
        if (!entry.undoable || entry.undone || !entry.before)
            continue;
...
function playerPositionClass(index, mode) {
    if (mode === 'yonma')
        return ['pos-bottom', 'pos-right', 'pos-top', 'pos-left'][index];
    if (index === 2)
        return core.config.sanmaThirdSeat === 'top' ? 'pos-top' : 'pos-left';
...
function renderLog() {
    ...
    if (!journal.length)
        list.innerHTML = '<div class="empty-state">目前沒有紀錄</div>';
}
...
function renderSettings() {
    ...
    document.querySelector(`input[name="length"][value="${c.gameLength}"]`).checked = true;
    $('#start-yonma').value = String(c.startYonma);
    $('#start-sanma').value = String(c.startSanma);
    $('#rule-kiriage').checked = c.rules.kiriageMangan;
    $('#rule-atamahane').checked = c.rules.atamahane;
    $('#rule-kazoe').checked = c.rules.kazoeYakuman;
    $('#rule-multiple-yakuman').checked = c.rules.multipleYakuman;
    document.querySelector(`input[name="sanma-third-seat"][value="${c.sanmaThirdSeat}"]`).checked = true;
    $('#center-text-scale').value = String(c.centerTextScale);
    ...
    names.querySelectorAll('[data-player-name-index]').forEach(input => {
...
function openPlayerSheet(player) {
    ...
    $('#action-ron').disabled = game.ended;
    $('#action-tsumo').disabled = game.ended;
    $('#action-draw').disabled = game.ended;
...
function declareSelectedRiichi() {
...
function scoreControlHtml(prefix, title) {
    return `
...
function readScoreInput(prefix) {
    const limit = $(`#${prefix}-limit`).value;
    return {
        limit,
        han: Number($(`#${prefix}-han`).value || 1),
        fu: Number($(`#${prefix}-fu`).value || 30),
        yakumanMultiplier: Number($(`#${prefix}-yakuman`).value || 1),
    };
}
function scorePreviewText(input, winner, tsumo) {
...
function bindScoreControls(prefix, winner, tsumo) {
    const update = () => {
        ...
        $(`#${prefix}-han`).disabled = input.limit !== 'normal';
        $(`#${prefix}-fu`).disabled = input.limit !== 'normal';
        $(`#${prefix}-preview`).textContent = scorePreviewText(input, winner, tsumo);
    };
    document.querySelectorAll(`[data-score-update="${prefix}"]`).forEach(el => el.addEventListener('input', update));
    update();
}
function openTsumoSheet() {
    closeSheets();
...
function openRonSheet() {
  ...
  `;
    if (core.config.rules.atamahane) {
        ...
        game.scores.forEach((_, i) => {
            const checkbox = document.querySelector(`input[data-winner="${i}"]`);
            if (!checkbox)
                return;
            const row = document.querySelector(`[data-winner-row="${i}"]`);
            const optional = row.querySelector('.optional-score');
            if (checkbox.checked || i === primaryWinner)
                bindScoreControls(`ron-${i}`, i, false);
    ...
    const loserSelect = $('#ron-loser');
    loserSelect.addEventListener('change', () => {
        const loser = Number(loserSelect.value);
        document.querySelectorAll('input[data-winner]').forEach(cb => {
            const idx = Number(cb.dataset.winner);
            if (idx === primaryWinner) {
                if (loser === idx)
                    loserSelect.value = String(game.scores.map((_, j) => j).find(j => j !== idx) ?? 0);
                return;
            }
    ...
    $('#confirm-ron').addEventListener('click', () => {
        const loser = Number($('#ron-loser').value);
        const entries = [];
        if (core.config.rules.atamahane) {
            ...
            document.querySelectorAll('input[data-winner]').forEach(cb => {
                ...
                if (cb.checked && winner !== loser)
                    entries.push({ winner, score: readScoreInput(`ron-${winner}`) });
            });
        }
...
function openDrawSheet() {
    ...
    `;
        const update = () => {
            const checked = [...document.querySelectorAll('input[data-tenpai]')].filter(x => x.checked);
            document.querySelectorAll('input[data-tenpai]').forEach(cb => {
                const row = cb.closest('.tenpai-row');
                row.classList.toggle('active', cb.checked);
                row.querySelector('.tenpai-state').textContent = cb.checked ? '聽牌' : '未聽';
            });
        ...
        document.querySelectorAll('input[data-tenpai]').forEach(cb => cb.addEventListener('change', update));
        $('#draw-back').addEventListener('click', renderMenu);
        $('#confirm-draw').addEventListener('click', () => {
            const tenpai = [...document.querySelectorAll('input[data-tenpai]')].filter(x => x.checked).map(x => Number(x.dataset.tenpai));
            runAction('DRAW', '一般流局', () => {
    ...
    `;
        document.querySelectorAll('input[data-nagashi]').forEach(cb => cb.addEventListener('change', () => {
            const row = cb.closest('.tenpai-row');
            row.classList.toggle('active', cb.checked);
            row.querySelector('.tenpai-state').textContent = cb.checked ? '成立' : '未選';
        }));
        $('#draw-back').addEventListener('click', renderMenu);
        $('#confirm-nagashi').addEventListener('click', () => {
            const winners = [...document.querySelectorAll('input[data-nagashi]')]
                .filter(x => x.checked)
                .map(x => Number(x.dataset.nagashi));
            const dealerTenpai = $('#nagashi-dealer-tenpai').checked;
...
function updateConfig(key, value, label) {
    runAction('SETTING', label, () => {
        core.config[key] = value;
        return label;
    });
}
function updateRule(key, value, label) {
    runAction('SETTING', label, () => {
...
function applyManualScores() {
    const values = [...document.querySelectorAll('[data-score-index]')]
...
function startNewGame() {
    ...
    if (!window.confirm(`新開 ${modeLabel(c.mode)}・${lengthLabel(c.gameLength)}？目前對局會保留在 Log，可用「上一步」復原。`))
        return;
...
function bindStaticEvents() {
    ...
    document.querySelectorAll('input[name="mode"]').forEach(input => input.addEventListener('change', () => {
        if (input.checked)
            updateConfig('mode', input.value, `下次新局模式：${modeLabel(input.value)}`);
    }));
    document.querySelectorAll('input[name="length"]').forEach(input => input.addEventListener('change', () => {
        if (input.checked)
            updateConfig('gameLength', input.value, `下次新局場次：${lengthLabel(input.value)}`);
    }));
    document.querySelectorAll('input[name="sanma-third-seat"]').forEach(input => input.addEventListener('change', () => {
        if (input.checked)
            updateConfig('sanmaThirdSeat', input.value, `三麻第三家位置：${input.value === 'top' ? '上方' : '左方'}`);
    }));
    $('#center-text-scale').addEventListener('input', () => {
        const value = Number($('#center-text-scale').value);
    ...
    $('#clear-log-button').addEventListener('click', () => {
        if (!window.confirm('只清除 Log？目前分數與局況不會改變。'))
            return;
